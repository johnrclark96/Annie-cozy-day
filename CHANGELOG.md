...
### Phase V · Visible-impact pivot
- V10: Music variation rotation. `MUSIC_MOODS` now includes a `variations` field per mood — three index-permutations into the existing `chords` array per mood (e.g. morning `[[0,1,2,3], [3,2,1,0], [1,3,0,2]]`). Same chord palette, different progression order = audibly different ambient music without doubling the data. New `store.musicVariation = { morning: 0, day: 0, evening: 0, night: 0, backyard: 0 }` with backfill. `setMusicMood` advances the relevant index `(idx+1) % variations.length` *before* re-starting ambient, persists via `saveJSON("musicVariation", …)`. `_scheduleMusicBeat` resolves `chord = mc.chords[mc.variations[idx][chordIdx]]` (falls back to `mc.chords[chordIdx]` if `variations` is missing — extra safety for future mood configs). The player hears a different progression each time the time-of-day or scene transition triggers a mood swap.
- V9: Settings panel + audio volume rails + reduced-motion gate. New `store.settings = { music: 1, sfx: 1, reducedMotion: false }` with backfill at the migration block (`:601–605`). New `audio.setVolume(music, sfx)` API on `CozyAudio` — `_sfxVol` is applied inside `envGain` (single chokepoint for tone/vibratoTone/noisePop), `_musicVol` multiplies the music master gain ramp in `startAmbient` and live-updates the running master node when the player drags the slider. New gear chip (`this.gearButton`) at chip-stack slot 5 (`y = SAFE + 170`). New `SETTINGS_PANEL = { x: 130, y: 90, w: 540, h: 420 }` panel using `drawPanelFrame`. Panel rows: Music slider, SFX slider, Reduced motion toggle, Reset tutorial button. Click anywhere on a track jumps the volume to that position; toggle flips on click; reset tutorial sets `tutorialSeen=false` and re-opens the V8 overlay; click outside the panel or Esc closes. Reduced-motion gate wires into `petSpriteState` (`:5180–5188`): when on, breath/tail amplitudes revert toward the pre-V4 values (obi/luna breath 0.022 → 0.012 / 0.022 → 0.01, tail 1.4/0.85 → 1.0/0.6). Audio is initialized from `store.settings` at startup so behavior is unchanged for existing players.
- V8: First-visit tutorial overlay (wires the V1908 orphan). New `store.tutorialSeen` boolean (defaults false via `loadBool`, no migration needed since the loadBool default handles old saves). On first hub entry from title (`store.stats.totalSessions <= 1 && !this.dedication`), set `this.tutorial = { phase: 0 }`. Render path adds a `drawIntroModal` call after the dedication block in `HangoutScene.draw`. Click anywhere or Esc dismisses + sets `tutorialSeen=true; saveBool` + plays `audio.tinyChime()`. New `helpButton` chip at chip-stack slot 4 (`y = SAFE + 136`) with a "?" glyph — clicking it re-opens the tutorial overlay anytime. Wired into `interactiveAt`, hover, click, and Esc cascade. The plan called for 4 numbered hint arrows; this commit ships the modal + chip, with arrows deferable as a follow-up.
- V7: Status strip demote + clickable-pill cursor. Bottom status strip (`:8536–8562`) — fill `rgba(255,255,255,0.9)` → `rgba(255,248,240,0.55)` cream so the strip stops competing with the pets it describes; stroke alpha 0.18 → 0.14 lineWidth 1.5 → 1; shine alpha 0.2 → 0.08 (was a bright white slash across the strip top); pulse glow alpha 0.15 → 0.08 (still celebrates but doesn't shout); text 15px → 13px Fredoka. Net: the strip recedes from "loudest thing on screen" to "supportive caption." `interactiveAt` (`:5471`) gains coin/star pill checks so the cursor switches to pointer over those pills, paying off the V3 chevron hint that they're clickable nav.
- V6: Decoration prominence — drop shadows + top-edge highlights for the four most-on-wall decor items the user might not realize they own. `d.photoWall` (`:4221`) frames get a 2-3px offset shadow under each frame plus a 1px cream highlight on the top edge. `d.corkBoard` (`:4242`), `d.wallClock` (`:4285`), `d.familyPortrait` (`:4347`) get the same treatment. Each item now reads as an object hung on the wall instead of a flat sticker. Targeted four items in this commit (instead of all 30) to keep the change reviewable; remaining decor (musicBox, garland, candles, etc.) can extend in a follow-up if these land well.
- V5: Static character shadow cache + thought-bubble outline. (a) `buildStaticCaches` now bakes an 80×24 soft elliptical shadow sprite into `sceneCache.charShadow`. `drawDynShadow` (`:8192–8210`) replaces the per-frame `createRadialGradient` (3× per frame at 60Hz = 180 allocs/sec) with a single `c.drawImage(sceneCache.charShadow, …)` modulated by `globalAlpha = shadowAlpha / 0.18` so the existing dynamic alpha curve is preserved. A tighter inner contact-shadow ellipse (`rgba(0,0,0,0.18)`, 4px hard) renders underneath for crisper grounding. (b) `drawThoughtBubble` (`:7901`) — added `c.shadowColor = "rgba(80,55,35,0.35)"; c.shadowBlur = 2;` right before the icon block so bubble icons (heart, bone, star, sparkle, food/water sprites) and the label text gain a subtle dark drop shadow, legible against busy decorated backgrounds. (c) Floating reaction text (`:8330–8344`) — strokeStyle changed from `rgba(255,255,255,0.85)` (white outline that smeared on the cream room) to `rgba(80,55,35,0.45)` (cocoa outline), lineWidth 3.5 → 3.
- V4: Pet idle motion amplification — bumped existing signal amplitudes and sprite multipliers so Obi and Luna read as alive instead of waiting. `drawAnnieSprite` (`:2388/2405`): bob multiplier 2.4 → 4.5 and stretchY 0.015 → 0.03 (visibly bigger breath cycle). `drawObiSprite` (`:2436`): stretchY multiplier 0.04 → 0.08 (more pronounced bounce squash on pet). `drawLunaSprite` (`:2470`): earTwitch rotation 0.04 → 0.09 (head tilts noticeably during ear flicks). `petSpriteState` (`:5176/5179/5193`): obiBreathe 0.012 → 0.022 (bumped `bounce` amplitude that drives both bob and stretchY), obi tail amp `0.6/1.0` → `0.85/1.4` (mood threshold `joy > 65` keeps the same boundary), lunaBreathe 0.01 → 0.022. `blinkSignal` (`:14777`): blink window widened from `mod < 0.14` → `mod < 0.20` and `mod > 2.4 && mod < 2.53` → `mod > 2.4 && mod < 2.62` (~40% longer blinks read as deliberate instead of frame-flicker). All numeric — no save changes. The `store.settings.reducedMotion` gate added in V9 will revert these by zeroing the petSpriteState outputs at source.
- V3: HUD hierarchy redesign — replaced the five-pill `placePillStack("tr", 1, [68, 72, 60, 60, 68])` row with a single right-anchored row at slot 1 driven by an items list. Coin pill (primary, `w=80`) gets a stronger gold outline, left-aligned digits, and a faint right-edge `▸` chevron suggesting it's clickable nav. Star pill (secondary, `w=60`) is lower-contrast cream with its own chevron. The conditional pills (streak, daily, weekly) only push their widths into the items list when their condition matches, so the row collapses leftward instead of reserving dead space for hidden pills. Hierarchy is now visible at-a-glance via width: coin (80) > star (60) > streak/weekly (50) > daily (44). `this.hubHudRects` is reduced to `{ coin, star }` (the streak/daily/weekly rects were never used by handlers). Coin popup re-anchored to the new coin pill center. Hover state on coin and star bumps fill alpha (a small V7 down payment). Replaces ~100 LOC with ~115 LOC; net visible change is "the right edge stops competing with itself."
- V1: Room depth pass — baked ground/contact shadows into `sceneCache.livingRoomBase` so furniture sits on the floor instead of floating on a flat plane. New depth pass right after the baseboard at `:3378–3404` draws palette-aware shadows: 8px elliptical shadow under the couch base (`400, 384`), 4px under the side table (`250, 380`), 6px under the cat tower (`698, 484`), 4px outer ring shadow just outside the rug ellipse (`400, 472`, r=224×58), 3px wall-shadow band under the bookshelf (`568, 244, 70`), 3px wall-shadow under the window sill (`56, 218, 140`). Couch front gets a 3px underside band at `(296, 374, 212)` after the legs are drawn; bookshelf gets a 2px bottom underside band at `(568, 240, 70)` after the body fill. Color per palette: `rgba(60,40,25,0.20)` for warm presets, `rgba(40,50,80,0.18)` for moonlight blue (preset 3) — switched at `(store.decor.roomPreset || 0) === 3`. Cache invalidates on decor toggle and `roomPreset` change (already wired) so cost is one-time per cache rebuild. Visible in every scene that calls `drawLivingRoom` (hub + dedication overlay + four minigames). Replaces "paper-cutout flatness" with grounded furniture without touching runtime draw loops.
- V2: Mode button active state — `drawButton` (`:1665`) gets a 6th `selected = false` parameter. When selected: button rect lifts by 2px (`y -= 2`), fill overrides to deep cocoa `#7A4E36`, a 3px outer cream ring `rgba(255,248,240,0.85)` is drawn 2px outside the button as a halo, the top inner shine and hover glow are skipped. Mode button call site (`~:7983`) replaces the dual-color hack `active ? "#A05A3C" : "#C7A37B"` with a single call passing `selected: this.mode === btn.key` against the standard `#C7A37B` fill. Resolves the "can't tell which mode is active at a glance" complaint that has been there since launch — the active mode now reads as a clearly-distinct dark cocoa pill with a cream halo, ~2× more contrast than the prior `#A05A3C` vs `#C7A37B` 25% delta. Non-mode buttons (Games, Decor, Closet, Resume, Quit, Play Again, Back, Start) default `selected = false` and render unchanged.

### Phase F · Living-systems surfacing
- F.NEW.3: Snow weather Luna idle. When `store.weather.current === "snow"` and Luna is leaving the floor for a perch, she now picks the windowsill 70% of the time instead of always defaulting to her tower. Three new status lines specific to snow ("Luna is watching the snow fall.", etc.). Tiny pet-personality detail that surfaces a system that previously had no behavioral consequence.
- F.8: Lucky Charm bonus surfaced on the minigame results screen. When the player owns the "Lucky Charm" upgrade, a faint caps line "includes +2 from Lucky Charm" renders below the "+N coins" total, so they can see what the +150-coin upgrade is actually doing for them. (The plan called for floating text inline; the results screen render is the natural single place — every minigame finish flows through `drawResults`.)
- F.7: New "Tasks" tab in the Scrapbook panel showing today's 3 daily tasks (each with a green checkbox or grey-empty state) and the active weekly challenge with a progress bar (0 → target). Added `{ key: "tasks", label: "Tasks" }` to `SCRAPBOOK_TABS` (now 5 tabs — `drawPanelTabs` re-flows to 5 equal-width pills automatically). Tab body reads from `store.dailyTasks.tasks/completed` and `store.weeklyChallenge`. Surfaces systems that previously only existed in the HUD pills (where they competed for attention with everything else).
- F.6: Visitor banner with countdown ring on top-center, plus first-appearance beat for seasonal visitors. New `_visitorBanner = { text, timer, total, isFirst }` instance state set in `spawnVisitorEvent`. Renders a 380×36 dark pill at y=78 for 2s (3s on first appearance), with a clockwise gold countdown ring drawn off `timer / total`. New `store.visitorsSeen` array tracks which visitor.keys have been seen — seasonal visitors (bunny, ice cream truck, scarecrow, Santa elf) get an upgraded "FIRST APPEARANCE" eyebrow + dark pill the very first time they appear in a new save. Existing `statusText` set kept for the bottom status bar so the spawn still has a bottom-line entry; the new banner is the prominent moment.
- F.5 + F.NEW.2: Welcome banner gets 20-line mood-aware rotation, plus streak-break welcome-back line. New top-level `WELCOME_LINES` array of 20 entries tagged with mood ("playful" / "cuddly" / "lazy" / "hungry" / "curious" / "aloof" / "affectionate" / "mischievous" / "any"). On title→hangout transition, the entry pool filters to lines matching either pet's daily mood plus generics; a random one becomes the welcome `statusText`. Replaces the prior 4-mood × 2-pet hardcoded matrix that sounded like flavor text. New `store.streakBreakPending` boolean (defaults false) flips to true in `recordCareAction` when a `>= 3-day` streak gets reset (`daysDiff > 2`); next title→hangout transition shows one of three soft-toned lines ("Obi looked for you yesterday." / "Luna sat by the door for a while." / "The pets missed your visit yesterday.") and clears the flag. Tone deliberately observational, never accusatory.
- F.4: Care-streak count surfaced on the title screen, just above the start button. Renders "🔥 N-day care streak" only when `store.careStreak.count >= 1`. Color escalates: muted brown at 1–6 days, warm orange `#D17B3A` at 7–29, gold `#E8A020` at 30+. Streak ribbons at 7/14/30-day milestones are already covered by the existing `decorNotification` banner that fires from the milestone block in HangoutScene.enter (idempotent via `careStreak.milestonesClaimed`); no separate `streakRibbonsClaimed` key needed since the existing system already prevents re-fires.
- F.3: Weather chip in the top-right HUD chip stack at slot 3 (y=SAFE+102), below sound / camera / scrapbook. New `this.weatherButton` rect on HangoutScene; hover sets `hoverKey = "weather"` with a tooltip showing the active weather label ("Sunny" / "Cloudy" / "Raining" / "Snowing" / "Golden hour"). Click opens the Scrapbook panel with the Goals tab pre-selected — natural for the "Rainy Day" / "Let It Snow" weather-keyed scrapbook goals which were previously invisible until the player happened to open Scrapbook. Icon rendered procedurally per weather (sun rays, cloud, rain droplets, snowflake spokes) — no atlas frames needed.
...
### Phase E · Heart tier
- E.6: 30-day "Dedicated Caretaker" reveal. The existing 30-day care-streak milestone path now queues `this._dedicatedReveal = { timer: 2.5, total: 2.5 }` and plays a single soft `audio.chord([261.63, 329.63, 392, 523.25], 0.8, 0.10)` chime. New render branch in HangoutScene draw renders a full-canvas cream ribbon at y=H/2 with eyebrow "DAY 30", title "Dedicated Caretaker", and subtitle "Annie has been here every day. Obi and Luna know." Three characters cheer in front of the ribbon (Annie cheer pose at x=320, Obi excited at x=410, Luna sitting at x=490, all bobbing on game.time). Fade-in 0.4s, hold, fade-out 0.4s. Replaces the existing flat "Dedicated Caretaker unlocked!" scrapbook entry with the dedication-themed sentence. Update tick decrements the timer alongside `_bondLevelUp`.
- E.5: "Maximum Cozy" hero shot in Cuddle Pile gets a soft fade. The win-state path already holds for 2.3s before transitioning to results, with the three sleeping poses already in place. The "Maximum Cozy!" overlay text now fades in over 0.6s, holds, then fades out over the last 0.3s of the hold so the crossfade to results feels soft rather than a hard cut. BG dim raised from 0.18 to 0.32 alpha so the freeze-frame reads as "the world has paused for this moment." Subtitle color shifted from `#B84B3A` (red) to `#7A4E36` (warm brown) — same as the title family.
- E.4: Anniversary re-trigger of the dedication card. Two new store keys: `firstVisitDate` (ISO yyyy-mm-dd, defaults to null for pre-E.4 saves so they don't get an anniversary they didn't earn), `lastDedicationYear` (defaults to 0). HangoutScene constructor now checks: if today's month+day match `firstVisitDate`'s month+day AND `today.year > lastDedicationYear` AND today's year isn't the same as the first-visit year, re-show the dedication and update `lastDedicationYear`. `firstVisitDate` is stamped at first dismissal (both click and Esc paths), so a new player today gets the dedication once now and again on this date next year. The first-time-only branch is gated on `_ddWasFirst = store.firstVisit` so a re-triggered anniversary doesn't double-record the scrapbook milestone.
- E.3: Replaced the 8 decorative floating hearts on the dedication card with a single rising heart from Annie's hand position. Anchor: Annie's sprite center (320, 370) at scale 0.8 + hand offset (+18, -38) → spawn at (338, 332). Heart appears at phase 0.6 (matches the "For Annie." line entry), rises at 60px/s, and fades over 1.5s. Subtle pink glow circle behind the heart for visibility against the cream paper bg. Reads as "Annie made this" rather than confetti. Anchor coords are the refined-plan E.7 specifics.
...
- D.NEW: Final closeBtn cleanup. Migrated the BackyardScene Decor panel to `drawPanelFrame` + `drawPanelClose` (last hand-coded panel). Replaced the remaining 3 hard-coded `{ x: W / 2 + 200, y: 84, w: 36, h: 36 }` close-button literals with `panelClose("decor")` calls (HangoutScene `interactiveAt` decor branch, BackyardScene `onClick` byDecor branch, BackyardScene hover handler). All panel close hits now read from a single source of truth — adding a new panel only needs to register its rect, not duplicate close geometry. Backyard decor "Coins:" subtitle dropped (HUD pill duplicates it). Title "Backyard Decor" font 26px → 28px (uses helper). Close button moves from (618, 102) center to (678, 92) center to match every other migrated panel.
- D.7: Scrapbook Goals tab gets actionable "how to" hints. Added a `hint` field to all 12 entries in `SCRAPBOOK_GOALS` — short, concrete how-to lines (e.g. "Click visitors when they appear", "On a 3-star card, click the gold star top-right to enter challenge mode"). Goal render line replaces `goal.desc` with `goal.hint || goal.desc`. The 12 goals fit in the existing 31px-per-row layout (155 → 527, within the 530 ceiling). The "wait for the right moment, take the photo" loop is now learnable from inside the panel rather than from outside knowledge.
